...
def main():
    ...
    parser.add_argument(
        "--image", type=str, help="Path to the image file or folder"
    )
    parser.add_argument(
        "--prompt",
        type=str,
    ...
    if is_folder:
        laion_ai_aesthetic_score = (
            laion_ai_aesthetic_predictor.evaluate_folder_aesthetic_score(
                args.image
            )
        )
    else:
    ...
    clip_score = clip_score_predictor.evaluate_clip_score(
    ...
    if is_folder and os.path.isdir(args.reference):
        arcface_distance = (
            arcface_distance_predictor.evaluate_folder_arcface_distance(
                args.reference, args.image
            )
        )
    elif args.reference:
        arcface_distance = (
            arcface_distance_predictor.evaluate_arcface_distance(
                args.reference, args.image
            )
        )
    else:
